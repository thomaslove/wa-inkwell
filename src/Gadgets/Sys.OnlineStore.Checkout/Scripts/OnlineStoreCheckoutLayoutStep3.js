(function (window, WA, undefined)
{
    ...
    function OnlineStoreCheckoutLayoutStep3(service, initModel)
    {
      ...
      var self = this, model, captchaVerify = false,
        ...
        $paymentProcessingFeeDonationCheckBox = jq$('#paymentProcessingFeeDonationCheckBox'),
      ...
      {                                        
        ...
        service.getConfirmModel().done(function(confirmModel) {
          ...
          displayPaymentProcessingFeeDonationCheckBox(model);
        });
      }
      ...
      function displayPaymentProcessingFeeDonationCheckBox(model)
      {
         if (!$paymentProcessingFeeDonationCheckBox.length)
           return;

         if(model.cart && model.cart.total === 0)
         {
           jq$('#paymentProcessingFeeDonation').hide(); 
         }   

         var amount = model.cart.total * initModel.paymentProcessingFeeDonationPercent / 100 + initModel.paymentProcessingFeeDonationIncrease;
         jq$('#paymentProcessingFeeDonationAmount').html(initModel.currencySymbol + amount.toFixed(2));
      }
      ...
      function patchModel(model)
      {
        ...
        model.paymentProcessingFeeDonationChecked = $paymentProcessingFeeDonationCheckBox.length && $paymentProcessingFeeDonationCheckBox.val() === 'on';
        return model;
      }
