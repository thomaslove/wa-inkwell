  ...
  function getTableRow(item) {
    ...
    return render(
      ...
      '<td class="firstTd">' + (item.isEventVisible
        ...
        '<div><span client-tz-item="{{timeZone}}" client-tz-formatted="{{formattedTimeZone}}">{{shortStartDate}}</span></div>' +
      '</td>' +
  ...
  function fetchRegistrations(filter, searchQuery, page) {
    WA.Ajax({
      ...
      success: function (data) {
        BonaPage.setPageState(BonaPage.PAGE_LOADING);
        ...
        BonaPage.setPageState(BonaPage.PAGE_LOADED);
      },
