    ...
    InvoiceListRenderer.Initialize = function()
    {
        ...
        function collectCheckboxesData(rebindDomElements)
        {
            var checkBoxesIds = InvoiceListRenderer.checkBoxeIds,
                    ...
                    checkedAmount = 0,
                    amountToPay = 0,
                    amountForPaymentProcessingFeeDonation = 0,
                    creditAmount = 0,
                    countToPay = 0,
                    selectedAllDocIds = [],
                    selectedDocs = [],
                    ...
                    disabledAmount = 0,
                    hiddenAmount = 0,
                    scope = <$Model.InvoiceAndPaymentList.PaymentProcessingFeeDonationScope$>;
            ...
            for (i = 0; i < len; i++)
            {
                ...
                if (docType == 1)
                {
                    ...
                    var origin = InvoiceListRenderer.ListHandler.DataSource.find(e => e[0] == item[4])[20];

		    if ((scope & 1) == 1 && (origin == 1 || origin == 2 || origin == 3)
                        || (scope & 2) == 2 && origin == 4
                        || (scope & 4) == 4 && origin == 100
                        || (scope & 16) == 16 && origin == 0)  
                        amountForPaymentProcessingFeeDonation += item[3];
                    countToPay++;
            ...
            return {
                ...
                amountForPaymentProcessingFeeDonation: Math.round(amountForPaymentProcessingFeeDonation * 100) / 100,  
        ...
        InvoiceListRenderer.RecurringInvoiceOnlyMessageBoxGroup.style.display = (checkboxesData.countToPay == 0) ? '' : 'none';
            ...
            if (InvoiceListRenderer.TopMessageBoxHeader)
            {
                ...
                {
                    var amountToPay = (Math.max(0.0, checkboxesData.amountToPay - checkboxesData.creditAmount)).toFixed(2);
                    InvoiceListRenderer.TopMessageBoxHeader.innerHTML = WA.String.format("<$Model.InvoiceAndPaymentList.Text.TotalBalanceTopMessageTemplate$>", checkboxesData.checkedAmount)
                            ...
                            + WA.String.format("<$Model.InvoiceAndPaymentList.PositiveCurrencyTemplate$>", amountToPay)
                            + "</span>";
                    
                   if (InvoiceListRenderer.PaymentProcessingFeeDonationPlaceHolder)
			InvoiceListRenderer.PaymentProcessingFeeDonationPlaceHolder.style.display =  checkboxesData.amountForPaymentProcessingFeeDonation ? '' : 'none';

                   if (InvoiceListRenderer.PaymentProcessingFeeDonationLabel)
                        InvoiceListRenderer.PaymentProcessingFeeDonationLabel.innerHTML = WA.String.format("<$Model.InvoiceAndPaymentList.Text.PaymentProcessingFeeDonationTemplate$>",
                            WA.String.format("<$Model.InvoiceAndPaymentList.PositiveCurrencyTemplate$>", 
                               (checkboxesData.amountForPaymentProcessingFeeDonation * 
				<$Model.InvoiceAndPaymentList.PaymentProcessingFeeDonationPercent$> / 100 + <$Model.InvoiceAndPaymentList.PaymentProcessingFeeDonationIncrease$>).toFixed(2)));
                }
        ...
        InvoiceListRenderer.ListHandler.ListRenderer.OnRenderComplete = function()
        {
            ...
            InvoiceListRenderer.PaymentProcessingFeeDonationLabel = document.getElementById("paymentProcessingFeeDonationLabel");
            InvoiceListRenderer.PaymentProcessingFeeDonationPlaceHolder = document.getElementById("paymentProcessingFeeDonationPlaceHolder");
